...
mofa_opalin <- run_mofa(
  mofa_opalin,
  outfile = "mofa_opalin_modelv4.hdf5",
  use_basilisk = TRUE
)
```
...
Reloading in the model to produce figure ( 2 views, Opalin vs Bulk oligo)
```{r}
library(MOFA2)
model <- load_model("/scratch/prj/bcn_marzi_lab/analysis_cutandtag_pd_sc/student_data_package/jd_analysis_sc/data_out/mofa_opalin_model.hdf5")
...
```
TEST 2 OPALIN+ AND BULK (2000 PEAKS)

```{r}
# bulk
bulk_vars    <- apply(bulk_counts, 1, var)
bulk_top_idx <- head(order(bulk_vars, decreasing = TRUE), 2000)
bulk_hvf     <- bulk_counts[bulk_top_idx, ]

# opalin
opalin_vars    <- apply(opalin_vst, 1, var)
opalin_top_idx <- head(order(opalin_vars, decreasing = TRUE), 2000)
opalin_hvf     <- opalin_vst[opalin_top_idx, ]
...
mofaplot_variance_explained(mofa_all, max_r2 = 15)
```
